...
idIsBiInv : ∀{i} (j : Size< i) {G : GlobSet i} {{_ : Composable G}} {{_ : HCat G}} → (x : cells G) → BiInvertible j Orig (id j x)
...
record BiInvertComp {h i : Size} {G : GlobSet h} {{_ : Composable G}} (j : Size< i) (A B C : Descendant G i) {x x' : cells (realise A)} {y y' : cells (realise B)} {z z' : cells (realise C)} (composition : GlobSetMorphism (morphisms (realise A) j x x' ×G morphisms (realise B) j y y') (morphisms (realise C) j z z')) : Set₁ where
  coinductive
  field
    biComp : {k : Size< j} {f : cells (morphisms (realise A) j x x')} {g : cells (morphisms (realise B) j y y')} → BiInvertible j A f → BiInvertible j B g → BiInvertible j C (func composition (f , g))
    biCompHigher : (k : Size< j) → (f f' : cells (morphisms (realise A) j x x')) → (g g' : cells (morphisms (realise B) j y y')) → BiInvertComp k (Child A j x x') (Child B j y y') (Child C j z z') (funcMorphisms composition k (f , g) (f' , g'))
