...
record Composable {i : Size} (G : GlobSet i) : Set where
  ...
  comp1 {j} {x} {y} {z} g f = func (comp j x y z) (g , f)
  comp2 : ∀{j : Size< i} {k : Size< j} {x y z : cells G} {g g' : cells (morphisms G j y z)} {f f' : cells (morphisms G j x y)} → cells (morphisms (morphisms G j y z) k g g') → cells (morphisms (morphisms G j x y) k f f') → cells (morphisms (morphisms G j x z) k (comp1 g f) (comp1 g' f'))
  comp2 {j} {k} {x} {y} {z} {g} {g'} {f} {f'} α β = func (funcMorphisms (comp j x y z) k (g , f) (g' , f')) (α , β)
  comp3 : ∀{j : Size< i} {k : Size< j} {l : Size< k} {x y z : cells G} {g g' : cells (morphisms G j y z)} {f f' : cells (morphisms G j x y)} → {α α' : cells (morphisms (morphisms G j y z) k g g')} → {β β' : cells (morphisms (morphisms G j x y) k f f')} → cells (morphisms (morphisms (morphisms G j y z) k g g') l α α') → cells (morphisms (morphisms (morphisms G j x y) k f f') l β β') → cells (morphisms (morphisms (morphisms G j x z) k (comp1 g f) (comp1 g' f')) l (comp2 α β) (comp2 α' β'))
  comp3 {j} {k} {l} {x} {y} {z} {g} {g'} {f} {f'} {α} {α'} {β} {β'} δ ε = func (funcMorphisms (funcMorphisms (comp j x y z) k (g , f) (g' , f')) l (α , β) (α' , β')) (δ , ε)
