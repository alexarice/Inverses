...
\begin{definition}
  Given a globular set \(G\) with identities and composition, with an \(n > 0\) cell \(f : x \to y\), a \emph{quasi-invertible} structure on \(f\) is:
  \begin{itemize}
  \item An \(n\)-cell \(\inv f : y \to x\);
  \item An \((n+1)\)-cell \(f_R: f \circ \inv f \to \id_y\);
  \item An \((n+1)\)-cell \(f_L: \inv f \circ f \to \id_x\).
  \item A quasi-invertible structure on \(f_R\).
  \item A quasi-invertible structure on \(f_L\).
  \end{itemize}
\end{definition}

\begin{remark}
  The previous definition is a coinductive one. Formally, we have defined a coinductive data type, which we could call \(\text{QuasiInvertible}(f)\), which references itself (in saying that \(f_R\) and \(f_L\) themselves have a quasi-invertible structure). Note that as \(\text{QuasiInvertible}\) is dependent on the parameter \(f\), it is in fact a coinductive type family as described in \Cref{sec:coinduction}.
\end{remark}
...
To end this section we give a small lemma, which demonstrates the use of these higher pre-categories and will also be invaluable later.
...
\begin{lemma}\label{lem:inverse-invert}
  Suppose \(f\) is quasi-invertible. Then so is \(\inv f\).
\end{lemma}

\begin{proof}
  Let \((\inv f, f_R, f_L, f_R{}I, f_L{}I)\) be a quasi-invertible structure on \(f\). Then \((f , f_L, f_R, f_L{}I, f_R{}I)\)  that \(\inv f\) is invertible.
\end{proof}
...
\end{lemma}\todo{We are defining \emph{one} bi-invertible structure on these. Use a definition and a lemma}
\begin{proof}
...
It should be noted that this is quite a natural way to define a cell between these structures. We simply defined a cell for each part of the structure separately, with pretty much only one way of defining each such cell. The only oddity may be that we require these cells to be bi-invertible. This is simply so that \((\id_f \circ_1 \phi_R) \circ f_R'\) has a canonical bi-invertible structure. Given this, the construction can be continued. Suppose we have \((\phi_R, \psi_R, BI_R, \phi_L, \psi_L, BI_L)\) and \((\phi_R', \psi_R', BI_R', \phi_L', \psi_L', BI_L')\). Then a cell is:
\begin{itemize}
...
\begin{theorem}
  Given a cell \(f : x \to y\), there is a cell (as described above) between any two bi-invertible structures.
\end{theorem}
...
\begin{proof}
  ...
  Using \Cref{thm:bi-inv-to-hai}, we can obtain a right-adjoint invertible structure \((\rinv f{}', f_R', \beta, f_R'H, \beta{}H)\). Then let \(\phi_R\) be the cell given by the diagram:
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\(\rinv f\)};
      \node [on grid, above left=1cm and 2.25cm of Bottom,draw](Cup){\(\beta\)};
    ...
    \end{tikzpicture}
  \end{center}
  This is clearly bi-invertible as it a composition of various types of invertible cell. We now let \(\psi_R\) be:
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\(\rinv f\)};
      \node [on grid, above left=1cm and 2.25cm of Bottom,draw](Cup){\(\beta\)};
