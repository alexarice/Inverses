  ...
  \item \(\beta_f\) is given by the following diagram:
    \begin{center}
      \begin{tikzpicture}
        ...
        \node [on grid, below right=40pt and 15pt of Finv, draw] (FRInv) {\(\rinv {f_R}\)};
        \node [on grid, above right=30pt and 30pt of FRInv, draw] (FL) {\(f_L\)};
        \node [on grid, below left=55pt and 25pt of FL, draw] (FLInv) {\(\linv {f_L}\)};
        \draw (Finv) to ++(0,-25pt) to[out=-90,in=180] (FRInv);
        \draw (FRInv) to[out=0,in=-90] ++(15pt,15pt) to[out=90,in=180] (FL);
        \draw (FL) to[out=0,in=90] ++(15pt,-15pt) to[out=-90,in=0] (FLInv);
  ...
  \item \(\gamma_f\) is given by the following diagram:
    \begin{center}
      \begin{tikzpicture}
        ...
        to (G'3);
      \end{tikzpicture}\todo{this diagram}
...
\item a bi-invertible 1-morphism from:
  \begin{center}
    \begin{tikzpicture}
      \node (Base1) {\(f\)};
      \node [on grid, right=45pt of Base1] (Base2) {\(\rinv f\)};
      \node [on grid, above=30pt of Base2, draw] (G1) {\(\phi_R\)};
      \node [on grid, above left=30pt and 22.5pt of G1, draw] (Top1) {\(f_R'\)};
      \draw (Base1) to ++(0,30pt)[out=90,in=180] to (Top1);
      \draw (Base2) to (G1);
      \draw (G1) to[out=90, in=0] (Top1);

      \node [on grid, right=30pt of G1,font=\fontsize{15}{24}\selectfont](M1){\(\Rightarrow\)};
      \node [above=0pt of M1]{\(\epsilon\)};

      \node [on grid, below right=30pt and 30pt of M1](Base3) {\(f\)};
      \node [on grid, right=45pt of Base3] (Base4) {\(\rinv f\)};
      \node [on grid, above=30pt of Base4, draw] (G2) {\(\phi_R'\)};
      \node [on grid, above left=30pt and 22.5pt of G2, draw] (Top2) {\(f_R'\)};
      \draw (Base3) to ++(0,30pt)[out=90,in=180] to (Top2);
      \draw (Base4) to (G2);
      \draw (G2) to[out=90, in=0] (Top2);

      \node [on grid, right=30pt of G2,font=\fontsize{15}{24}\selectfont](M2){\(\Rightarrow\)};
      \node [above=0pt of M2] {\(\psi_R'\)};

      \node [on grid, below right=30pt and 30pt of M2](Base5) {\(f\)};
      \node [on grid, right=45pt of Base5] (Base6) {\(\rinv f\)};
      \node [on grid, above right=60pt and 22.5pt of Base5,draw] (Top3) {\(f_R\)};
      \draw (Base5) to ++(0cm,30pt)[out=90,in=180] to (Top3);
      \draw (Base6) to ++(0cm,30pt)[out=90,in=0] to (Top3);
    \end{tikzpicture}
  \end{center}\todo{resize diagram}
  to \(\psi_R\);
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [on grid, above left=50pt and 15pt of Bottom, draw](Cap){\(f_R\)};
      \node [on grid, above left=20pt and 45pt of Cap](Top){\(\rinv f{}'\)};
      \draw (Bottom) to ++(0,30pt) to[out=90,in=0] (Cap);
      \draw (Cap) to[out=180,in=90] ++(-15pt,-15pt) to[out=-90,in=0] (Cup);
      \draw (Cup) to[out=180,in=-90] ++(-15pt,15pt) to (Top);
    \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw (Bottom) to ++(0,30pt) to[out=90,in=0] (Cap);
      \draw (Cap) to[out=180,in=90] ++(-15pt,-15pt) to[out=-90,in=0] (Cup);
      \draw (Cup) to[out=180,in=-90] ++(-15pt,15pt) to[out=90,in=0] (Top);
      \draw (Top) to[out=180,in=90] ++(-15pt,-15pt) to (End);

      \node [on grid, above right=20pt and 20pt of
      Bottom,font=\fontsize{15}{24}\selectfont] (M1)
      {\(\Rightarrow\)};
      \node [above=0pt of M1] {\(\gamma_f\)};

      \node [on grid, below right=20pt and 20pt of M1](Base5) {\(f\)};
      \node [on grid, right=30pt of Base5] (Base6) {\(\rinv f\)};
      \node [on grid, above right=40pt and 15pt of Base5,draw] (Top3)
      {\(f_R\)}; \draw (Base5) to ++(0,20pt)[out=90,in=180] to
      (Top3); \draw (Base6) to ++(0cm,20pt)[out=90,in=0] to (Top3);
    \end{tikzpicture}
  \end{center}
