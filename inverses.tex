...
\usepackage{amsthm} \usepackage{hyperref} \usepackage{cleveref}[capitalise]
...
\(\mathsf{headProof}(xs) = \mathsf{is{\text -}even}(\mathsf{head}(xs))\) and
...
one could show that these conditions hold, then the results should
follow for free.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \node [on grid, above=40pt of A] (Top1) {};
    \node [on grid, right=30pt of Base1](Base2) {};
    \node [on grid, draw, above=40pt of Base2] (B) {\(\beta\)};
    \node [on grid, above=20pt of B] (Top2) {};
    \draw (Base1) to (A);
    \draw (A) to (Top1);
    \draw (Base2) to (B);
    \draw (B) to (Top2);
  \end{tikzpicture}
  \caption{\((\alpha \comp_1 \id) \comp_0 (\id \comp_1 \beta)\)}
...
that can be required, yet each of these implies the other and so it
...
\begin{proof}
  ...
  structure \((\rinv f{}', f_R', \beta_{f'}, \gamma_{f'}, {f_R'}^{HAI}, \beta_{f'}^{HAI}, \gamma_{f'}^{HAI})\) can be obtained.
