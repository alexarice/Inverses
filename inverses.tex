...
\usepackage{tikz} \usepackage{listings} \usepackage{color}
...
\begin{abstract}
  ...
  We define some properties we expect any reasonable definition of a
  weak \(\omega\)-category to possess. With these properties we define
  3 notions of invertibility inspired by type theory. These are
  ...
  to show that these 3 notions are all equivalent in that given any
...
and so the usual definition states that there is a natural isomorphism
between them.
...
\comment{Should there be something on sized types, which are used in
  the agda formalisation?}
...
complicated \todo{add citation}. Therefore, similar in style to a
...
\begin{definition}
  ...
  codimension of the composition. \comment{explain what this is?}
\end{definition}
...
diagrams, the source and target morphisms are equal\todo{add
  citation}\todo{add example}.
...
\begin{lemma}\label{composition-lemma}
  ...
  we can generate a pseudo-bi-invertible structure on \(f \comp_n g\).
\end{lemma}
\begin{proof}
  Split into cases on \(n\):
  \begin{itemize}
  \item \(n = 0\): Let \((\rinv f, \linv f, f_R, f_L, f_R{}BI,
    f_L{}BI)\) and \((\rinv g, \linv g, g_R, g_L, g_R{}BI, g_L{}BI)\)
    be bi-invertible structures for \(f\) and \(g\) respectively. We
    define a pseudo-bi-invertible structure on \(g \comp_0 f\) where:
    \begin{itemize}
    ...
    \item \({(g \comp_0 f)}_R : g \comp_0 f \comp_0 \rinv f \comp_0
      ...
      \begin{center}
        \begin{tikzpicture}
          \node[draw] (GR) {\(g_R\)}; \node[on grid, below=1 of GR,
          draw] (FR) {\(f_R\)}; \node[on grid, below left=1.5cm and
          2.25cm of FR] (Base1) {\(g\)}; \node[on grid, right=0.75cm
          of Base1] (Base2) {\(f\)}; \node[on grid, right=3cm of
          Base2] (Base3) {\(\rinv f\)}; \node[on grid, right=0.75cm of
          Base3] (Base4) {\(\rinv g\)}; \draw (Base1) to ++(0,1cm) to
          [out=90,in=180] (GR); \draw (Base4) to ++(0,1cm) to
          [out=90,in=0] (GR); \draw (Base2) to ++(0,0.75cm) to
          [out=90,in=180] (FR); \draw (Base3) to ++(0,0.75cm) to
          [out=90,in=0] (FR);
        \end{tikzpicture}
      \end{center}
    \item \({(g \comp_0 f)}_L : \linv g \comp_0 \linv f \comp_0 f
      ...
      \begin{center}
        \begin{tikzpicture}
          \node[draw] (GL) {\(g_L\)}; \node[on grid, below=1 of GL,
          draw] (FL) {\(f_L\)}; \node[on grid, below left=1.5cm and
          2.25cm of FL] (Base1) {\(\linv g\)}; \node[on grid,
          right=0.75cm of Base1] (Base2) {\(\linv f\)}; \node[on grid,
          right=3cm of Base2] (Base3) {\(f\)}; \node[on grid,
          right=0.75cm of Base3] (Base4) {\(g\)}; \draw (Base1) to
          ++(0,1cm) to [out=90,in=180] (GL); \draw (Base4) to
          ++(0,1cm) to [out=90,in=0] (GL); \draw (Base2) to
          ++(0,0.75cm) to [out=90,in=180] (FL); \draw (Base3) to
          ++(0,0.75cm) to [out=90,in=0] (FL);
        \end{tikzpicture}
      \end{center}
    \end{itemize}
...
\begin{proof}
  ...
  \item \({(\rinv f)}_R: \rinv f \comp_0 f \to \id\) is the morphism:
    ...
    \begin{center}
      \begin{tikzpicture}
        \node (Finv) {\(\rinv f\)}; \node [on grid, right=1cm of Finv]
        (F) {\(f\)}; \node [on grid, above left=2cm and 0.75cm of
        Finv, draw] (FR) {\(f_R\)}; \node [on grid, below left=1.5cm
        and 1.5cm of FR, draw] (FLInv) {\(\rinv {f_L}\)}; \node [on
        grid, above right=2.75cm and 1.25cm of FLInv, draw] (FL)
        {\(f_L\)}; \draw (Finv) to ++(0,1.25cm) to[out=90,in=0] (FR);
        \draw (FR) to[out=180,in=90] ++(-0.75cm,-0.75cm)
        to[out=-90,in=0] (FLInv); \draw (FLInv) to[out=180,in=-90]
        ++(-0.75cm,0.75cm) to[out=90,in=180] (FL); \draw (F) to
        ++(0,1.25cm) to[out=90,in=0] (FL);
      \end{tikzpicture}
    \end{center}
...
one of these\todo{Put a reference for this, or a proof, or both}. This
is what gives rise to the name \emph{half}-adjoint invertible. In
\Cref{sec:contractibility}, both coherence conditions will be useful
...
\begin{definition}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\({f'}\)}; \node[on grid, above left=1cm and
      2.25cm of Bottom, draw] (Cup){\(\beta_f\)}; \node[on grid, above
      right=2cm and 1.5cm of Cup,draw] (Cap){\(\alpha_f\)}; \node[on
      grid, above left=1cm and 2.25cm of Cap] (Top) {\({f'}\)}; \draw
      (Bottom) to++(0,2) to[out=90,in=0] (Cap); \draw (Cup)
      to[out=0,in=-90] ++(0.75,1) to[out=90,in=180] (Cap); \draw (Cup)
      to[out=180,in=-90] ++(-0.75,1) to (Top);

      \node[on grid, above right=2cm and 1cm of Bottom,
      font=\fontsize{20}{24}\selectfont] (Eq) {\(\Rightarrow\)};
      \node[above=0cm of Eq,font=\fontsize{15}{19}\selectfont] (Gamma)
      {\(\gamma_f\)}; \node[on grid, right=2cm of Bottom] (L1)
  ...
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\({f'}\)}; \node[on grid, above right=1cm and
      2.25cm of Bottom, draw] (Cup){\(\beta_f\)}; \node[on grid, above
      left=2cm and 1.5cm of Cup,draw] (Cap){\(\alpha_f\)}; \node[on
      grid, above right=1cm and 2.25cm of Cap] (Top) {\({f'}\)}; \draw
      (Bottom) to++(0,2) to[out=90,in=180] (Cap); \draw (Cup)
      to[out=180,in=-90] ++(-0.75,1) to[out=90,in=0] (Cap); \draw
      (Cup) to[out=0,in=-90] ++(0.75,1) to (Top);

      \node[on grid, below right=2cm and 1cm of Top,
      font=\fontsize{20}{24}\selectfont] (Eq) {\(\Rightarrow\)};
      \node[above=0cm of Eq,font=\fontsize{15}{19}\selectfont] (Gamma)
      {\(\gamma_f\)}; \node[on grid, right=2cm of Top] (L1)
...
\begin{theorem}\label{bi-inv-to-hai}
  ...
  a cell \(f\) induces a right adjoint invertible structure \((\rinv
  f, f_R, \dots)\) on \(f\).
\end{theorem}
\begin{proof}
  Let \((\rinv f, \linv f, f_R, f_L, f_R^{BI}, f_L^{BI})\) be a
  bi-invertible structure on \(f\). Then we give the right-adjoint
  invertible structure \((\rinv f, f_R, \beta_f, \gamma_f, f_R^{RAI},
  \beta_f^{RAI}, \gamma_f^{RAI})\) where:
  \begin{itemize}
  \item \(\beta_f\) is given by the following diagram:
    \begin{center}
      \begin{tikzpicture}
        \node (Finv) {\({\rinv f}\)}; \node [on grid, right=1cm of
        Finv] (F) {\(f\)}; \node [on grid, below left=2cm and 0.75cm
        of Finv, draw] (FRInv) {\(\rinv {f_R}\)}; \node [on grid,
        above left=1.5cm and 1.5cm of FRInv, draw] (FL) {\(f_L\)};
        \node [on grid, below right=2.75cm and 1.25cm of FL, draw]
        (FLInv) {\(\linv {f_L}\)}; \draw (Finv) to ++(0,-1.25cm)
        to[out=-90,in=0] (FRInv); \draw (FRInv) to[out=180,in=-90]
        ++(-0.75cm,0.75cm) to[out=90,in=0] (FL); \draw (FL)
        to[out=180,in=90] ++(-0.75cm,-0.75cm) to[out=-90,in=180]
        (FLInv); \draw (F) to ++(0,-1.25cm) to[out=-90,in=0] (FLInv);
      \end{tikzpicture}
    \end{center}
  \item \(\gamma_f\) is given by the following diagram:
    \begin{center}\comment{possible improvements to this diagram}
      \begin{tikzpicture}
        \node (G) {\({\rinv f}\)}; \node [on grid, below right=0.25cm
        and 1.75cm of G, draw] (FR) {\(f_R\)}; \node [on grid, below
        right=5cm and 0.75cm of FR] (G') {\({\rinv f}\)}; \node [on
        grid, below left=3cm and 0.75cm of G, draw] (FRInv) {\(\rinv
          {(f_R)}\)}; \node [on grid, above left=1.5cm and 1.5cm of
        FRInv, draw] (FL) {\(f_L\)}; \node [on grid, below
        right=2.75cm and 1.25cm of FL, draw] (FLInv) {\(\linv
          {(f_L)}\)}; \draw (G) to ++(0,-2.25cm) to[out=-90,in=0]
        (FRInv); \draw (FRInv) to[out=180,in=-90] ++(-0.75cm,0.75cm)
        to[out=90,in=0] (FL); \draw (FL) to[out=180,in=90]
        ++(-0.75cm,-0.75cm) to[out=-90,in=180] (FLInv); \draw (FR)
        to[out=180,in=90] ++(-0.75cm,-0.75cm) to ++(0,-1.25cm)
        to[out=-90,in=0] (FLInv); \draw (FR) to[out=0,in=90]
        ++(0.75cm,-0.75cm) to (G');

        \node [on grid, below right=2.75cm and 3.75cm of
        G,font=\fontsize{20}{24}\selectfont] (A1) {\(\Rightarrow\)};
        \node [on grid, above=0.5cm of A1] (A1Text) {isotopy};

        \node [on grid, right=8cm of G] (G1) {\({\rinv f}\)}; \node
        [on grid, below left=2cm and 0.75cm of G1, draw] (FRInv1)
        {\(\rinv {(f_R)}\)}; \node [on grid, below=1cm of FRInv1,
        draw] (FR1) {\(f_R\)}; \node [on grid, above left=1.5cm and
        1.5cm of FRInv1, draw] (FL1) {\(f_L\)}; \node [on grid, below
        left=1.5cm and 1.5cm of FR1, draw] (FLInv1) {\(\linv
          {(f_L)}\)}; \node [on grid, below right=2cm and 0.75cm of
        FR1] (G'1) {\({\rinv f}\)}; \draw (G1) to ++(0,-1.25cm)
        to[out=-90,in=0] (FRInv1); \draw (FRInv1) to[out=180,in=-90]
        ++(-0.75cm,0.75cm) to[out=90,in=0] (FL1); \draw (FL1)
        to[out=180,in=90] ++(-0.75cm,-0.75cm) to ++(0,-2.5cm)
        to[out=-90,in=180] (FLInv1); \draw (FR1) to[out=180,in=90]
        ++(-0.75cm,-0.75cm) to[out=-90,in=0] (FLInv1); \draw (FR1)
        to[out=0,in=90] ++(0.75cm,-0.75cm) to (G'1);

        \node [on grid, rotate=-90, below left=1.5cm and 1.5cm of
        G'1,font=\fontsize{20}{24}\selectfont] (A2) {\(\Rightarrow\)};
        \node [on grid, left=1cm of A2] (A2Text) {\((f_R)_R\)};

        \node [on grid, below=8cm of G1] (G2) {\({\rinv f}\)}; \node
        [on grid, below=3.5cm of G2] (G'2) {\({\rinv f}\)}; \node [on
        grid, below left=1cm and 2.25cm of G2, draw] (FL2) {\(f_L\)};
        \node [on grid, below=1.5cm of FL2, draw] (FLInv2) {\(\linv
          {(f_L)}\)}; \draw (G2) to (G'2); \draw (FL2) to[out=0,in=90]
        ++(0.75cm,-0.75cm) to[out=-90,in=0] (FLInv2); \draw (FL2)
        to[out=180,in=90] ++(-0.75cm,-0.75cm) to[out=-90,in=180]
        (FLInv2);

        \node [on grid, below left=0.75cm and 2.5cm of
        FL2,font=\fontsize{20}{24}\selectfont] (A3) {\(\Leftarrow\)};
        \node [on grid, above=0.5cm of A3] (A3Text) {\((f_L)_L\)};

        \node [on grid, below=8cm of G] (G3) {\({\rinv f}\)}; \node
        [on grid, below=3.5cm of G3] (G'3) {\({\rinv f}\)}; \draw (G3)
        to (G'3);
      \end{tikzpicture}
...
f_R^{BI}', f_L^{BI}')\). A cell\todo{give this a name?} is a tuple
...
BIC_R, \dots)\) and \((\phi_R', \psi_R', BIC_R', \dots)\). Then a cell
is:
\begin{itemize}
...
\item a bi-invertible cell from:
  \begin{center}
    \begin{tikzpicture}
      \node (Base1) {\(f\)}; \node [on grid, right=1.5cm of Base1]
      (Base2) {\(\rinv f\)}; \node [on grid, above=1cm of Base2, draw]
      (G1) {\(\phi_R\)}; \node [on grid, above left= 1cm and 0.75cm of
      G1, draw] (Top1) {\(f_R'\)}; \draw (Base1) to
      ++(0cm,1cm)[out=90,in=180] to (Top1); \draw (Base2) to (G1);
      \draw (G1) to[out=90, in=0] (Top1);

      \node [on grid, right=1cm of
      G1,font=\fontsize{20}{24}\selectfont](M1){\(\Rightarrow\)};
      ...
      \node [on grid, above left= 1cm and 0.75cm of G2, draw] (Top2)
      {\(f_R'\)}; \draw (Base3) to ++(0cm,1cm)[out=90,in=180] to
      (Top2); \draw (Base4) to (G2); \draw (G2) to[out=90, in=0]
      (Top2);

      \node [on grid, right=1cm of
      G2,font=\fontsize{20}{24}\selectfont](M2){\(\Rightarrow\)};
      ...
      \node [on grid, above right=2cm and 0.75cm of Base5,draw] (Top3)
      {\(f_R\)}; \draw (Base5) to ++(0cm,1cm)[out=90,in=180] to
      (Top3); \draw (Base6) to ++(0cm,1cm)[out=90,in=0] to (Top3);
    \end{tikzpicture}
  \end{center}
  to \(\psi_R\);
...
\begin{theorem}
  Given a cell \(f : x \to y\), there is a cell (as described above)
...
\begin{proof}
  ...
  Using \Cref{bi-inv-to-hai}, we can obtain a right-adjoint invertible
  structure \((\rinv f{}', f_R', \beta, f_R^{RAI}', \beta^{RAI})\).
  Then let \(\phi_R\) be the cell given by the diagram:
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\(\rinv f\)}; \node [on grid, above left=1cm and
      2.25cm of Bottom,draw](Cup){\(\beta\)}; \node [on grid, above
      left=2.5cm and 0.75cm of Bottom, draw](Cap){\(f_R\)}; \node [on
      grid, above left=1cm and 2.25cm of Cap](Top){\(\rinv f{}'\)};
      \draw (Bottom) to ++(0cm,1.5cm) to[out=90,in=0] (Cap); \draw
      (Cap) to[out=180,in=90] ++(-0.75cm,-0.75cm) to[out=-90,in=0]
      (Cup); \draw (Cup) to[out=180,in=-90] ++(-0.75cm,0.75cm) to
      (Top);
    \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node (Bottom) {\(\rinv f\)}; \node [on grid, above left=1cm and
      2.25cm of Bottom,draw](Cup){\(\beta\)}; \node [on grid, above
      right=1.5cm and 1.5cm of Cup, draw](Cap){\(f_R\)}; \node [on
      grid, above left=1.5cm and 1.5cm of Cup,draw](Top){\(f_R'\)};
      \node [on grid, left=4.5cm of Bottom] (End) {\(f\)}; \draw
      (Bottom) to ++(0cm,1.5cm) to[out=90,in=0] (Cap); \draw (Cap)
      to[out=180,in=90] ++(-0.75cm,-0.75cm) to[out=-90,in=0] (Cup);
      \draw (Cup) to[out=180,in=-90] ++(-0.75cm,0.75cm)
      to[out=90,in=0] (Top); \draw (Top) to[out=180,in=90]
      ++(-0.75cm,-0.75cm) to (End);

      \node [on grid, above right=1cm and 1cm of
      Bottom,font=\fontsize{20}{24}\selectfont] (M1)
      {\(\Rightarrow\)};
  ...
  cell is also left-adjoint invertible.\todo{Add proof of
    this}\todo{why is it invertible}
\end{proof}
...
\todo{citation}. These can be thought of as the next coherence up. It
